...
pub unsafe fn parse_c_str<'a>(ptr: *const c_char) -> Option<&'a str> { unsafe {
    if ptr.is_null() {
        return None;
    }
...
pub unsafe fn c_str_to_string(s: *const c_char) -> Result<String, BoxliteError> { unsafe {
    if s.is_null() {
...
pub unsafe fn free_c_string(s: *mut c_char) { unsafe {
    if !s.is_null() {
